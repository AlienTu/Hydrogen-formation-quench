...
function residualHalf(p1,p2,g,k1,k2,dx){
  const sp=Math.sqrt(Math.PI), beta=2*sp, Nh=p1.length, n=Nh-2, r=new Float64Array(2*n);
  let maxr=0, l2=0;
  for(let i=1;i<Nh-1;i++){
    ...
    r[j]=r1; r[j+n]=r2;
    maxr=Math.max(maxr,Math.abs(r1),Math.abs(r2)); l2+=r1*r1+r2*r2;
  }
  return{r,maxr,l2:Math.sqrt(l2)}
}

function applyJHalf(v,out,p1,p2,g,k1,k2,dx){
  const sp=Math.sqrt(Math.PI), beta=2*sp, Nh=p1.length, n=Nh-2, inv=1/(dx*dx), mix=0.5*g*g;
  for(let i=0;i<n;i++){
    const gi=i+1, u1=v[i], u2=v[i+n];
    const l1=i>0?v[i-1]:0, rr1=i<n-1?v[i+1]:0, l2=i>0?v[i+n-1]:0, rr2=i<n-1?v[i+n+1]:0;
    const m11=mix+2*Math.PI*k1*Math.cos(beta*p1[gi]);
    const m22=mix+2*Math.PI*k2*Math.cos(beta*p2[gi]);
    out[i]=(2*u1-l1-rr1)*inv+m11*u1+mix*u2;
    out[i+n]=(2*u2-l2-rr2)*inv+m22*u2+mix*u1;
  }
}

function cgSolveHalf(b,p1,p2,g,k1,k2,dx,maxIt,tol){
  const n=b.length, x=new Float64Array(n), r=new Float64Array(b), z=new Float64Array(n), p=new Float64Array(n), Ap=new Float64Array(n);
  // Jacobi preconditioner from local diagonal of the block-tridiagonal Jacobian.
  const Nh=p1.length, ni=Nh-2, inv=1/(dx*dx), sp=Math.sqrt(Math.PI), beta=2*sp, mix=0.5*g*g;
  for(let i=0;i<ni;i++){
    const gi=i+1;
    const d1=2*inv+mix+2*Math.PI*k1*Math.cos(beta*p1[gi]);
    const d2=2*inv+mix+2*Math.PI*k2*Math.cos(beta*p2[gi]);
    z[i]=r[i]/Math.max(Math.abs(d1),1e-12);
    z[i+ni]=r[i+ni]/Math.max(Math.abs(d2),1e-12);
  }
  p.set(z);
  let rz=dot(r,z), rz0=Math.max(Math.abs(rz),1e-300);
  for(let it=0;it<maxIt;it++){
    Ap.fill(0); applyJHalf(p,Ap,p1,p2,g,k1,k2,dx);
    const denom=dot(p,Ap);
    if(!Number.isFinite(denom)||Math.abs(denom)<1e-300)break;
    const a=rz/denom;
    for(let i=0;i<n;i++){x[i]+=a*p[i]; r[i]-=a*Ap[i];}
    if(norm(r)<tol*Math.sqrt(rz0))break;
    for(let i=0;i<ni;i++){
      const gi=i+1;
      const d1=2*inv+mix+2*Math.PI*k1*Math.cos(beta*p1[gi]);
      const d2=2*inv+mix+2*Math.PI*k2*Math.cos(beta*p2[gi]);
    ...
    const rzNew=dot(r,z), betaCG=rzNew/Math.max(rz,1e-300);
...
function solveBackground(g,k1,k2,L,Nh){
  const sp=Math.sqrt(Math.PI), x=range(-L,0,Nh), dx=x[1]-x[0];
  ...
  // A short relaxation only puts the kink-like ansatz into Newton's basin.
  const dtau=0.08*dx*dx;
  for(let it=0;it<300;it++){
    const n1=p1.slice(), n2=p2.slice();
    const rr=residualHalf(p1,p2,g,k1,k2,dx).r, ni=Nh-2;
    for(let i=1;i<Nh-1;i++){const j=i-1;n1[i]=p1[i]-dtau*rr[j];n2[i]=p2[i]-dtau*rr[j+ni];}
    p1=n1;p2=n2;p1[0]=0;p1[Nh-1]=sp/2;p2[0]=0;p2[Nh-1]=-sp/2;
  }

  let info=residualHalf(p1,p2,g,k1,k2,dx);
  for(let newt=0;newt<28;newt++){
    if(info.maxr<1e-9)break;
    const b=new Float64Array(info.r.length); for(let i=0;i<b.length;i++)b[i]=-info.r[i];
    const delta=cgSolveHalf(b,p1,p2,g,k1,k2,dx,Math.min(1200,8*b.length),1e-10);
    let accepted=false, bestP1=p1, bestP2=p2, bestInfo=info;
    for(let ls=0;ls<12;ls++){
      const lam=Math.pow(0.5,ls), t1=p1.slice(), t2=p2.slice(), ni=Nh-2;
      for(let i=1;i<Nh-1;i++){const j=i-1;t1[i]=p1[i]+lam*delta[j];t2[i]=p2[i]+lam*delta[j+ni];}
      t1[0]=0;t1[Nh-1]=sp/2;t2[0]=0;t2[Nh-1]=-sp/2;
      const ti=residualHalf(t1,t2,g,k1,k2,dx);
      if(ti.l2<bestInfo.l2){bestP1=t1;bestP2=t2;bestInfo=ti;}
      if(ti.l2<info.l2*(1-1e-4*lam)){accepted=true;break;}
    }
    p1=bestP1;p2=bestP2;info=bestInfo;
    if(!accepted && norm(delta)<1e-12)break;
  }
  return{x,p1,p2,dx,res:info.maxr,resL2:info.l2}
}

function mirror(bg){const sp=Math.sqrt(Math.PI),h=bg.x,p1=bg.p1,p2=bg.p2;return{x:h.concat(h.slice(0,-1).map(v=>-v).reverse()),p1:p1.concat(p1.slice(0,-1).map(v=>sp-v).reverse()),p2:p2.concat(p2.slice(0,-1).map(v=>-sp-v).reverse()),dx:bg.dx,res:bg.res,resL2:bg.resL2}}
...
function chooseModes(spec,lo,hi,topK){const chosen=[];for(let i=0;i<Math.min(6,spec.pairs.length);i++)chosen.push(i);spec.pairs.forEach((m,i)=>{if(m.w<lo)chosen.push(i)});const lowWindow=spec.pairs.map((m,i)=>[spec.iprs[i],i]).filter(p=>p[1]<Math.min(24,spec.pairs.length));lowWindow.sort((a,b)=>b[0]-a[0]).slice(0,Math.min(topK,4)).forEach(p=>chosen.push(p[1]));const firstLo=spec.pairs.findIndex(m=>m.w>lo); if(firstLo>=0)chosen.push(firstLo);const firstHi=spec.pairs.findIndex(m=>m.w>hi); if(firstHi>=0)chosen.push(firstHi);return Array.from(new Set(chosen)).sort((a,b)=>a-b)}
