...
function jacobiSym(A,maxSweeps=80,tol=1e-11){
  ...
  for(let sw=0;sw<maxSweeps;sw++){
    ...
    const tau=(aqq-app)/(2*apq); const t=Math.sign(tau||1)/(Math.abs(tau)+Math.sqrt(1+tau*tau));
  ...
  const vals=Array.from({length:n},(_,i)=>A[i][i]); return{vals,V}
}
...
function lanczos(op,kdim){const dim=2*op.n,qPrev=new Float64Array(dim),q=new Float64Array(dim),z=new Float64Array(dim);for(let i=0;i<dim;i++)q[i]=Math.sin(12.9898*(i+1))+0.1*Math.cos(78.233*(i+1));scale(q,1/norm(q));const Q=[],alpha=[],beta=[];let b=0;for(let j=0;j<kdim;j++){Q.push(new Float64Array(q));applyH(q,z,op);if(j>0)axpy(z,-b,qPrev);const a=dot(q,z);alpha.push(a);axpy(z,-a,q);for(let r=0;r<Q.length;r++){const c=dot(Q[r],z);axpy(z,-c,Q[r])}b=norm(z);if(j<kdim-1)beta.push(b);if(b<1e-12)break;qPrev.set(q);q.set(z);scale(q,1/b)}return{Q,alpha,beta,m:alpha.length}}
function tridiagEig(alpha,beta){const m=alpha.length,T=zeros(m,m);for(let i=0;i<m;i++){T[i][i]=alpha[i];if(i<m-1){T[i][i+1]=beta[i];T[i+1][i]=beta[i]}}const ev=jacobiSym(T,100,1e-10);const pairs=ev.vals.map((v,i)=>({w2:v,w:Math.sqrt(Math.max(v,0)),i})).sort((a,b)=>a.w-b.w);return{pairs,V:ev.V}}
...
function runSolver(){setStatus('Computing background...');setTimeout(()=>{try{const gin=+qs('g').value,k1=+qs('k1').value,k2=+qs('k2').value,L=+qs('L').value,Nh=Math.round(+qs('Nh').value),num=+qs('numModes').value,topK=+qs('topIpr').value,kdim=Math.round(+qs('krylovDim').value);const g=Math.sqrt(2)*gin;const vac=eig2(0.5*g*g+2*Math.PI*k1,0.5*g*g,0.5*g*g+2*Math.PI*k2),lo=Math.sqrt(vac[0]),hi=Math.sqrt(vac[1]);qs('lowerThr').textContent=lo.toFixed(4);qs('upperThr').textContent=hi.toFixed(4);const bg=mirror(solveBackground(g,k1,k2,L,Nh));qs('residual').textContent=bg.res.toExponential(2);drawBackgroundPlots(bg,g,k1,k2,vac[0],vac[1]);const dim=2*(bg.x.length-2);setStatus('Background plotted. Running matrix-free Lanczos: dim='+dim+', Krylov='+kdim+'...');setTimeout(()=>{try{const spec=spectrumLanczos(bg,g,k1,k2,num,kdim),sel=chooseModes(spec,lo,hi,topK);qs('selectedModes').textContent=sel.join(', ');Plotly.newPlot('spectrumPlot',[{x:spec.pairs.map((_,i)=>i),y:spec.pairs.map(m=>m.w),mode:'markers',marker:{size:spec.iprs.map(v=>7+260*v),color:spec.iprs,colorscale:'Viridis',showscale:true,colorbar:{title:'IPR'}}},{x:[0,spec.pairs.length-1],y:[lo,lo],mode:'lines',name:'lower threshold',line:{dash:'dot'}},{x:[0,spec.pairs.length-1],y:[hi,hi],mode:'lines',name:'upper threshold',line:{dash:'dot'}}],layout('Lanczos spectrum: marker size/color = IPR'),{responsive:true});const n=bg.x.length-2,xi=bg.x.slice(1,-1),modeTr=[],denTr=[];for(const j of sel){const m=spec.pairs[j],u=spec.vecs[j],arr=[];let mx=1e-12;for(let r=0;r<n;r++){const val=u[r]+u[r+n];arr.push(val);mx=Math.max(mx,Math.abs(val))}modeTr.push({x:xi,y:arr.map(v=>v/mx),name:'n='+j+', w='+m.w.toFixed(4)});const dm=Math.max(...spec.dens[j]);denTr.push({x:xi,y:spec.dens[j].map(v=>v/dm),name:'n='+j+', IPR='+spec.iprs[j].toFixed(3)})}plotLine('modePlot',modeTr,'Selected Lanczos-Ritz mode functions, all centered at 0');plotLine('densityPlot',denTr,'Selected mode densities');let html='<table><tr><th>n</th><th>omega</th><th>omega^2</th><th>IPR</th><th>W(|x|<5)</th><th>class</th></tr>';spec.pairs.forEach((m,i)=>{if(sel.includes(i)||i<8){const cl=m.w<lo?'below lower':(m.w<hi?'between':'above upper');html+=`<tr><td>${i}</td><td>${m.w.toFixed(5)}</td><td>${m.w2.toFixed(5)}</td><td>${spec.iprs[i].toFixed(4)}</td><td>${spec.w5[i].toFixed(3)}</td><td>${cl}</td></tr>`}});html+='</table>';qs('candidateTable').innerHTML=html;setStatus('Done. Lanczos used Krylov dimension '+spec.krylov+'. Increase Krylov dimension to improve high-index/upper-threshold candidates.')}catch(e){console.error(e);setStatus('Lanczos error: '+e.message)}} ,80)}catch(e){console.error(e);setStatus('Error: '+e.message)}},50)}
